  ...
  function render(){
    ...
    function getPlayerName(playerSymbol){
      if(playerSymbol === state.players[0].symbol)
        return state.players[0].isComputer ? 'IA' : "J1"
      return state.players[1].isComputer ? 'IA' : "J2"
    }
    ...
    function htmlQ1(){
      return `<div id="view1"><h3>Que choisissez-vous ?\n</h3>
      ${buttonHTML(1, "1player", "Joueur VS IA")}
      ${buttonHTML(1, "2players", "J1 VS J2")}
      </div>`
    }

    function htmlQ2(){
      const html2=`<div id="view2"><h3>${!state.players[1].isComputer? "J1, <br />" : ""}Quel symbole voudriez-vous utiliser ?</h3>
      ${buttonHTML(2, "X", "X")}
      ${buttonHTML(2, "O", "O")}`
      return html2
    }
    ...
    function htmlGame (){
      ...
      const playerName = state.game.turn === 0 ? 'J1' : state.players[1].isComputer ? 'IA' : 'J2'
    //   let playerName = 'Computer'
    //   if(!state.players[state.game.turn].isComputer)
    //     playerName = state.game.turn === 0 ? 'Player1' : 'Player2'

      let htmlBefore = `<h3>tour : ${moveNumber} ${htmlSpaces(5)} Au tour de : ${playerName}</h3>`
      let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
        ...
        let htmlAfter = `<h4>Score : ${htmlSpaces(1)} J1 - ${state.players[0].score} ${htmlSpaces(2)} ${state.players[1].isComputer? "IA" : "J2" } - ${state.players[1].score}</h4>`
    ...
    function htmlGameEnd (){
      ...
      if(result !== RESULT.tie)
        resultText = getPlayerName(result) + " l'emporte"

      let htmlBefore = `<h3>${resultText} ${htmlSpaces(2)} Cliquez sur un bouton pour recommencer </h3> `
      let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
